...
def read_subj(subj_dir, hemi, in_ch, data_norm):
    feat_list = []
    for ch in in_ch:
        f = os.path.join(subj_dir, hemi + "." + ch)
        feat_list.append(f)

    data = np.array([])
    for f in feat_list:
        feat, _ = read_feature(f)
...
def main(args):
    ...
    print("Loading data...")
    native_data = read_subj(
        subj_dir=args.subj_dir, in_ch=ckpt_args["in_ch"], hemi=args.hemi, data_norm=ckpt_args["data_norm"]
    )

    sphere = os.path.join(args.subj_dir, args.hemi + "." + args.native_sphere)
    ...
    out_prefix = out_prefix[-2] + "." + args.hemi if len(out_prefix) > 1 else args.hemi
    out_file = out_prefix + ".label.txt"
    np.savetxt(os.path.join(args.out_dir, out_file), parc, delimiter=",", fmt="%d")
